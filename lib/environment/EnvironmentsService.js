...
class EnvironmentsService extends BaseService {
  ...
  update(id, data, done) {
    ...
    // Allow the PUT call to update the environment settings to be temporarily bypassed to prevent our MIC settings from being unset.
    if (dataKeys && dataKeys.length === 1 && dataKeys[0] === 'name') {
      return done();
    }
